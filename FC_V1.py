#Writing function for getting house_number, borough, street_name, year and odd_even from all violations CSV
...
def ticketprocess(pid,records):
    ...
    for row in reader:
        ...
        yield house_number,year,Y2015,Y2016,Y2017,Y2018,Y2019,boro,street_name,odd_even,year 
...
def streetprocess(pid,records):
    ...
    for row in reader:
        ...
        yield physical_id,ll_house_number,lh_house_number,street_name,st_name,boro_code,'odd'
        yield physical_id,rl_house_number,rh_house_number,street_name,st_name,boro_code,'even'
...
if __name__ == "__main__":
    ...
    final_df = centerline_df.join(broadcast(violations_df),
...
#counts3 = counts.map(lambda x:((x[2],x[3]),(x[0],x[1],x[4])))
#counts4 = counts2.map(lambda x:((x[4],x[3]),(x[0],x[1],x[2],x[5])))
#counts5 = counts3.join(counts4).collect()
#counts6 = counts5.map(lambda x:((x[0][0], x[0][1]), (x[1][0],x[1][1],x[1][2], x[2][0],x[2][1],x[2][2]))).collect()
